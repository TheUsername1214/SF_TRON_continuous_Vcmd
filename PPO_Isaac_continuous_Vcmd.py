...
class PPO:
    def __init__(self, bot_cfg, train):
        ...
        self.Kp = FT(bot_cfg.RobotParam.Kp)
        self.Kd = FT(bot_cfg.RobotParam.Kd)
    ...
    def scene_setup(self):
        ...
        self.action = torch.zeros(size=(self.agents_num, self.actuator_num) ,device=self.device)
    ...
    def get_store_observation(self, current_step):
        ...
        current_privilege = torch.concatenate(
            (self.body_height.view(-1, 1),
             ...
             self.L_feet_air_time.view(-1, 1)*20,
             self.R_feet_air_time.view(-1, 1)*20
             ), dim=1)
        # #——————————————————————储存状态————————————————————————————————##
    ...
    def apply_PD_target(self, angle):
        ...
        self.effort2 += torch.sum(torch.clip(torque * joint_vel, 0, 11111), dim=1) / self.sub_step
        self.effort3 += torch.sum(torch.abs(joint_vel)**4, dim=1) / (self.sub_step * 8)
        self.time += self.dt/self.sub_step
    ...
    def get_store_state_reward_over(self, current_step):
        ...
        # ——————————————————————奖励————————————————————————————————
        # 速度跟踪
        reward_vel_track = 0.5 * exp_sum(self.vel_cmd[:, :2],
        ...
        reward_ori_track = 0.1 * exp_sum(self.target_ori[:, 2],
                                         self.next_body_ori[:, 2], 0.1, 1)
        reward_ori_track += 0.1 * exp_sum(self.target_ori[:, 0],
                                          self.next_body_ori[:, 0], 0.1, 1)
        reward_ori_track += 0.05 * exp_sum(self.target_ori[:, 1],
                                          self.next_body_ori[:, 1], 0.1, 1)
        ...
        # 奖励SF
        walking_phase_reward = 0.2 * (3*(self.next_L_foot_contact_situation != self.next_R_foot_contact_situation).float()-2)
        ...
        # 惩罚扭矩使用
        joint_effort_reward = -0.002 * self.effort2

        # 动作平滑
        action_smooth_reward = -0.06 *torch.abs(self.action-self.previous_action).flatten()

        # 惩罚失败
        over1 = torch.any(torch.abs(self.next_body_ori) > np.pi / 6, dim=1)
        ...
        reward = reward_fall + \
                 ...
                 +feet_air_time + \
                 + foot_regularization_reward + joint_effort_reward+0.3
        # ——————————————————————奖励结束————————————————————————————————#
        ...
        self.action_smooth_reward_sum += action_smooth_reward.mean()
    ...
    def play(self):
        ...
        for epi in range(self.episode):
            ...
            if self.train:
                ...
                print("action_smooth_reward_sum",self.action_smooth_reward_sum.item() / self.maximum_step)
